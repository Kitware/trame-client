...
export async function registerUserScripts() {
    ...
    const external_scripts = window.trame.state.state.trame__client_external_scripts || [];
    ...
    for (const external_script of external_scripts) {
        ...
        const get_module_fn = external_script.module_type === "es" ? loadEsModule : lookupUmdModule;

        const registering_module = get_module_fn(external_script.path).catch((e) => {
            throw new Error(`Could not load module ${external_script.path}. Did you pass the correct module path?`, {
                cause: e
            });
        }).then((module) => {
            register_handle({ module, module_path: external_script.path }, external_script.function, external_script.id);
        });

        module_registration_promises.push(registering_module);
    }

    const preload_results = await Promise.allSettled(module_registration_promises);
