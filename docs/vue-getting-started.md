# Getting Started with trame + Vue3 (Python Guide)

`client_type="vue3"` is trame's default, production client — everything in
...
`{{ ... }}` takes a JavaScript expression, evaluated against the current
...
Unlike a one-way `{{ }}` binding, `v_model` is already two-way: it both
...
scoped variable that `{{ item.name }}` and any other bindings inside the
...
- [`react-getting-started.md`](./adding-support-for-react/react-getting-started.md)
  — the equivalent guide for the (design-exploration, not yet implemented)
  React client, if you want a side-by-side syntax comparison
