...
//   3. trame__react_use : list of registry plugins to load
...
// ----------------------------------------------------------------------------
// Resource management (script/css/react_use)
// ----------------------------------------------------------------------------
...
export async function handlePageResources(state) {
  ...
  const useEntries = state.trame__react_use || [];
  return useEntries.map((libUseEntry) => {
    ...
    if (!lib) {
      console.error(
        `Lookup error: registry.use(${libKey}, ${JSON.stringify(libOptions)})`,
      );
      return;
    }
    return { name: libKey, plugin: lib, options: libOptions };
  });
}
